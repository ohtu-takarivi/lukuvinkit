function autofillbook(buttonautofillbookId, authorId, isbnId, titleId) {
    // disable the button while fetching
    $('#' + buttonautofillbookId).prop('disabled', true);
    // clean up the ISBN and do some quick validation
    var isbnClean = encodeURIComponent(document.getElementById(isbnId).value);
    ...
    $.ajax({
        ...
        complete: function(data) {
            $('#' + buttonautofillbookId).prop('disabled', false);
...
$(document).ready(function () {
    $('#buttonautofillbook').click(function(){
        autofillbook('buttonautofillbook', 'author', 'isbn', 'title');
    });
});
