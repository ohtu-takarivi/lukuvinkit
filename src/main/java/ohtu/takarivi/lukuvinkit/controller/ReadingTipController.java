...
public class ReadingTipController {
    ...
    //    List<ReadingTip> list = readingTipRepository.findByCustomUserIdAndTitleContaining(customUser.getId(), keyword);
        List<ReadingTip> list2 = readingTipRepository.findByTitleContainingAndCustomUserIdOrDescriptionContainingAndCustomUserId(keyword, customUser.getId(), keyword, customUser.getId());
